...
public class AlignmentTest {
  ...
  private static final AlignmentRule SUFFIX_ALIGNMENT_RULES =
      AlignmentRules.compose(
          ...
          AlignmentRules.constantForSuffix(".u", 1), AlignmentRules.constantForSuffix(".a", 1024));
  ...
  public void extraFieldSpaceUsedForAlignmentCanBeReclaimedBeforeUpdate() throws Exception {
    ...
    assertArrayEquals(
        recognizable2,
        readSegment(
            zipFile, ZFileTestConstants.LOCAL_HEADER_SIZE + "f.u".length(), recognizable2.length));
  }
  ...
  public void extraFieldSpaceUsedForAlignmentCanBeReclaimedAfterUpdate() throws Exception {
    ...
    assertArrayEquals(
        recognizable2,
        readSegment(
            zipFile, ZFileTestConstants.LOCAL_HEADER_SIZE + "f.b".length(), recognizable2.length));
  }

  @Test
  public void fillEmptySpaceWithExtraFieldAfterDelete() throws Exception {
    File zipFile = new File(temporaryFolder.getRoot(), "large.zip");
    ...
    try (ZFile zf = new ZFile(zipFile)) {
      ...
      assertEquals(0, entry.getCentralDirectoryHeader().getOffset());
      assertEquals(
          ZFileTestConstants.LOCAL_HEADER_SIZE + "first.u".length() + recognizable1.length,
  ...
  public void fillInLargeGapsWithExtraField() throws Exception {
    ...
    assertArrayEquals(
        recognizable1, readSegment(zipFile, recognizable1Start, recognizable1.length));
    ...
    int recognizable2Start =
        3 * ZFileTestConstants.LOCAL_HEADER_SIZE
            ...
            + bigEmpty.length;
    assertArrayEquals(
        recognizable2, readSegment(zipFile, recognizable2Start, recognizable2.length));
  }
  ...
  public void fourByteAlignment() throws Exception {
    ...
    try (ZFile zf = new ZFile(zipFile, options)) {
      ...
      long fooOffset = (long) ZFileTestConstants.LOCAL_HEADER_SIZE + fooName.length() + 7;
      ...
      long barStartOffset = (long) ZFileTestConstants.LOCAL_HEADER_SIZE + barName.length() + 6;
      ...
      long xptoStartOffset = (long) ZFileTestConstants.LOCAL_HEADER_SIZE + xptoName.length() + 9;
